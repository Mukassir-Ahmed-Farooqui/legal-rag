...
export const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      const detail = err.response?.data?.detail || 'Registration failed. Please check your credentials.';
      setError(detail);
      toast.error(detail);
    } finally {
      setLoading(false);
      ...
      <div className="w-full max-w-md space-y-8 bg-white p-8 rounded-xl shadow-lg border border-slate-100">
        <div className="flex flex-col items-center justify-center text-center">
          ...
          <p className="mt-2 text-sm text-slate-505">
            Get started with ClauseScope contract platform
          </p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="space-y-4 rounded-md">
            <div>
              <label htmlFor="full-name" className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">
                Full Name
              </label>
              <input
                id="full-name"
